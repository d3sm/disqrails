...
class ArticleMetadataFetcher
  ...
  def fetch_html(uri, redirects_remaining: MAX_REDIRECTS)
    response = Net::HTTP.start(uri.host, uri.port, use_ssl: uri.scheme == "https", open_timeout: 3, read_timeout: 4) do |http|
      http.get(uri.request_uri, { "User-Agent" => "hor-bot/0.1 (+https://news.ycombinator.com/)" })
    end
  ...
  def extract_meta(html)
    ...
    html.scan(/<meta\b[^>]*>/i).each do |tag|
      ...
      meta[:image] ||= content if %w[og:image twitter:image].include?(normalized_key)
      meta[:description] ||= content if %w[og:description twitter:description description].include?(normalized_key)
    end
